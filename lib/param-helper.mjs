import { Vector, Matrix } from '@kaosat-dev/sylvester/src/index';
import intersect from 'intersect';
import arrayIsEqual from 'array-equal';
...
function expandedParams(params) {
  ...
  params.angularMomentum = angularMomentumFromSemilatusRectum(params.semilatusRectum,params.mu);
...
function perifocalPosition(angularMomentum, eccentricity, trueAnomaly, mu) {
  ...
  return (
    Vector.create([Math.cos(theta), Math.sin(theta), 0]).x(
      (Math.pow(h, 2) / mu) *
...
function perifocalVelocity(angularMomentum, eccentricity, trueAnomaly, mu) {
  ...
  return (
    Vector.create([-Math.sin(theta), e + Math.cos(theta), 0]).x(
      (mu / h)
    )
  );
};
...
function baseParams(params) {
  ...
  if(typeof params.mu === 'undefined'){
    params.mu = constants.MU;
  }
  if(typeof params.centralBodyRadius === 'undefined'){
...
function angularMomentumFromSemilatusRectum(semilatusRectum,mu) {
